...
- **Every command must launch its script through an explicit interpreter — `pwsh -NoProfile -ExecutionPolicy Bypass -File "$s"` and `bash "$s"`, never `& $s` or a bare `"$s"`.** The guard below proves the file is *there*; it does not prove the OS will *run* it, and each platform refuses differently. `& $s` runs a script *file*, which PowerShell execution policy governs, and the harness passes no policy flag — on a stock Windows client (all scopes `Undefined` → `Restricted`) the script refuses to load and the hook exits 1. A bare `"$s"` *execs* the file on POSIX, which needs the executable bit that `[ -f "$s" ]` never checks — a copy that lost its mode bit fails with 126, which is how macOS users hit the same blanket denial. The validator enforces both exact forms. Neither beats a Group Policy scope; nothing process-scoped does. Note that an explicit interpreter does **not** rescue CRLF — see the `.sh` rule below.
