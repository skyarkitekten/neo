...
**Known drift** below and is _documented, not fixed_, by this spec.
...
| `.agent-hooks/log-event.sh`       | Both        | The one script both `hooks.json` files shell out to — hook _config_ is per-harness, hook _behavior_ is shared.                                           |
...
| `agents`      | _absent_ (Claude resolves agents from the conventional `agents/` path)                          | `".github/agents"`                           | **Copilot-specific**, required for Copilot.                      |
| `hooks`       | _absent_ (Claude resolves hooks from the conventional `hooks/hooks.json` path)                  | `".github/hooks/hooks.json"`                 | **Copilot-specific**, required for Copilot.                      |
...
- **Copilot CLI** reads `.github/plugin/{plugin,marketplace}.json` → agents from
  ...
  `${PLUGIN_ROOT}`).
- **Copilot's `.github/plugin/` tree is checked _before_ `.claude-plugin/`.** Because both
...
- **`marketplace.json`'s `source` field** — each harness's marketplace entry declares its own
  relative source path; the two are allowed to differ in _notation_ as long as they resolve to
...
- **Skills are not `neo`-prefixed.** The two skills in the repo today, `task-authoring`
  (`.github/skills/task-authoring/SKILL.md`) and `feature-authoring`
  (`.github/skills/feature-authoring/SKILL.md`), use a bare kebab-case name with no `neo-`
  prefix. Skill namespacing is provided by the plugin/marketplace name (`neo`) itself at install
  time, not embedded in the skill's own identifier — unlike agents, which are addressed by bare
  name within a session and so carry the prefix directly.
...
inside it _is_ kebab-case (`code-reviewer`, not `codeReviewer`). It is not a violation of the
...
Claude Code's `agents/` (7 files) and Copilot's `.github/agents/` (8 files) do not mirror each
other one-to-one:
...
| `neo-feature-agent.md`      | `neo.feature-agent.agent.md`          |
| `neo-planner.md`            | `neo.implementation-planner.agent.md` |
| _(none)_                    | `neo.task-planner.agent.md`           |

Six roles (code-reviewer, code-writer, master-control, researcher, technical-engineer,
feature-agent) mirror cleanly, one file per harness with matching content.
...
- **`neo-planner.md` (Claude) and `neo.implementation-planner.agent.md` (Copilot) are the same
  ...
  by unit…"). So the **Implementation Planner** role _does_ have a Claude-side file — it is
...
Net effect: Claude's crew is 7 files covering 7 roles; Copilot's crew is 8 files covering 8
roles (the same 7, plus Task Planner) — and one of the 7 shared roles (Implementation Planner)
